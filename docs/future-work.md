...
- **Run provenance stamps.** Record git commit, generator-manifest hash, and
  ...
  to exact inputs.
- **Python lock refresh cadence.** `constraints.txt` (added 2026-09-10) pins
  every CI and image install to one reviewed set; it is regenerated by hand
  with the command in its header, not by a bot, so a stale-pin check (e.g.
  `pip-audit -r constraints.txt` on the nightly) is the natural next step.
